...
export interface IdleToRespawnArgs {
  input: types.IdleToRespawnInputFields
}

export interface IdleToRespawnAccounts {
...
export const layout = borsh.struct([types.IdleToRespawnInput.layout("input")])

export function idleToRespawn(
  args: IdleToRespawnArgs,
  accounts: IdleToRespawnAccounts,
  ...
  const identifier = Buffer.from([235, 207, 200, 174, 161, 116, 91, 168])
  const buffer = Buffer.alloc(1000)
  const len = layout.encode(
    {
      input: types.IdleToRespawnInput.toEncodable(args.input),
    },
    buffer
  )
