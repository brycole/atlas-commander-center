...
export interface StopMiningAsteroidArgs {
  input: types.StopMiningAsteroidInputFields
}

export interface StopMiningAsteroidAccounts {
...
export const layout = borsh.struct([
  types.StopMiningAsteroidInput.layout("input"),
])

export function stopMiningAsteroid(
  args: StopMiningAsteroidArgs,
  accounts: StopMiningAsteroidAccounts,
  ...
  const identifier = Buffer.from([181, 77, 45, 163, 103, 27, 211, 81, 0, 0])
  console.log("-------------_>>>>>_>>_>_>_>>_A__"+identifier.toString('hex'))

  const buffer = Buffer.alloc(1000)
  const len = layout.encode(
    {
      input: types.StopMiningAsteroidInput.toEncodable(args.input),
    },
    buffer
  )
