...
export interface CreateCargoPodArgs {
  input: types.StarbaseCreateCargoPodInputFields
}

export interface CreateCargoPodAccounts {
...
export const layout = borsh.struct([
  types.StarbaseCreateCargoPodInput.layout("input"),
])

export function createCargoPod(
  args: CreateCargoPodArgs,
  accounts: CreateCargoPodAccounts,
  ...
  const identifier = Buffer.from([143, 62, 133, 80, 133, 83, 167, 17])
  const buffer = Buffer.alloc(1000)
  const len = layout.encode(
    {
      input: types.StarbaseCreateCargoPodInput.toEncodable(args.input),
    },
    buffer
  )
