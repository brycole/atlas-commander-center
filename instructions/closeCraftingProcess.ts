...
export interface CloseCraftingProcessArgs {
  input: types.StarbaseCloseCraftingProcessInputFields
}

export interface CloseCraftingProcessAccounts {
...
export const layout = borsh.struct([
  types.StarbaseCloseCraftingProcessInput.layout("input"),
])

export function closeCraftingProcess(
  args: CloseCraftingProcessArgs,
  accounts: CloseCraftingProcessAccounts,
  ...
  const identifier = Buffer.from([202, 21, 225, 156, 15, 4, 106, 93])
  const buffer = Buffer.alloc(1000)
  const len = layout.encode(
    {
      input: types.StarbaseCloseCraftingProcessInput.toEncodable(args.input),
    },
    buffer
  )
