...
export interface WithdrawCargoFromGameArgs {
  input: types.CargoToGameInputFields
}

export interface WithdrawCargoFromGameAccounts {
...
export const layout = borsh.struct([types.CargoToGameInput.layout("input")])

export function withdrawCargoFromGame(
  args: WithdrawCargoFromGameArgs,
  accounts: WithdrawCargoFromGameAccounts,
  ...
  const identifier = Buffer.from([102, 218, 88, 53, 255, 194, 24, 62])
  const buffer = Buffer.alloc(1000)
  const len = layout.encode(
    {
      input: types.CargoToGameInput.toEncodable(args.input),
    },
    buffer
  )
